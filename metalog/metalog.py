...
def fit(
    ...
    if ((type(x) is not list) and (type(x) is not np.ndarray)) or (
        not all(isinstance(b, (int, float, np.int_, np.float_)) for b in x)
    ):
        raise Exception(
            "Error: 'x' must be a numeric vector of type 'list' or 'numpy.ndarray'"
        )

    if ((type(bounds) is not list) and (type(bounds) is not np.ndarray)) or (
        not all(isinstance(b, (int, float, np.int_, np.float_)) for b in bounds)
    ):
        raise Exception(
            "Error: 'bounds' must be a numeric vector of type 'list' or 'numpy.ndarray'"
        )

    if term_limit % 1 != 0:
        raise Exception(
            "Error: 'term_limit' parameter should be an integer between 3 and 30"
        )
    ...
    if np.sum(np.isnan(probs)) == 0:
        ...
        if np.max(probs) > 1 or np.min(probs) < 0:
            raise Exception(
                "Error: 'probs' parameter must have values between (bun not including): 0 and 1"
            )
    ...
    if len(bounds) != 2 and boundedness == "b":
        raise Exception(
            "Error: 'bounds' must have two bounds: upper and lower as a numeric vector (i.e. [12, 45])"
        )

    if (np.max(bounds) < np.min(bounds)) and (boundedness == "b"):
        raise Exception(
            "Error: in 'bounds' upper bound must be greater than lower bound"
        )

    if (np.min(x) < np.min(bounds)) and (boundedness == "b"):
        raise Exception(
            "Error: lower bound in 'bounds' must be less than or equal to the smallest value of 'x'"
        )

    if (np.max(bounds) < np.max(x)) and (boundedness == "b"):
        raise Exception(
            "Error: upper bound in 'bounds' must be greater than or equal to the largest value of 'x'"
        )
    ...
    if (
        ...
        raise Exception(
            "Error: 'boundedness' parameter must be: 'u', 'su', 'sl' or 'b' only"
        )

    if (np.max(x) > bounds[1]) and (boundedness == "su"):
        raise Exception(
            "Error: for 'su' in 'boundedness' the upper bound must be greater than or equal to the largest value of 'x'"
        )

    if (np.min(x) < bounds[0]) and (boundedness == "sl"):
        raise Exception(
            "Error: for 'sl' in 'boundedness' the lower bound must be less than or equal to the smalles values of 'x'"
        )
    ...
    if term_limit > len(x):
        raise Exception(
            "Error: 'term_limit' must be less than or equal to the length of the vector 'x'"
        )

    if term_lower_bound > term_limit:
        raise Exception(
            "Error: 'term_lower_bound' must be less than or equal to 'term_limit'"
        )
    ...
    myList = support_func.a_vector_OLS_and_LP(
        myList,
    ...
    if save_data == True and term_lower_bound <= 3:
        ...
        Ys = pd.concat(
            [Ys, pd.DataFrame((s - 0.5) * np.array(Ys["y2"]), columns=["y3"])], axis=1
        )
...
def r(m, n=1, term=3):
    ...
    if (
        ...
        or (term < 2)
        or (term % 1 != 0)
        or (term not in valid_terms)
    ):
        raise Exception(
            "Error: term must be a single positive integer contained in the metalog object. Available terms are: "
...
def q(m, y, term=3):
    ...
    if (
        (not all(isinstance(b, (int, float, np.int_, np.float_)) for b in y))
        or (type(y) is not list)
        or (max(y) >= 1)
        or (min(y) <= 0)
    ):
        raise Exception(
            "Error: 'y' must be a positive numeric vector of 'numpy.ndarray'-type with values between 0 and 1"
        )
...
def p(m, q, term=3):
    ...
    qs = np.array(
        [support_func.newtons_method_metalog(m, q[i], term) for i in range(len(q))]
    )
    return qs
...
def d(m, q, term=3):
    ...
    ds = np.array(
        [support_func.pdfMetalog_density(m=m, y=qs[i], t=term) for i in range(len(qs))]
    )
...
def plot(m, norm=True):
    ...
    res_data = pd.DataFrame(
        {
            "term": np.repeat(
                str(m["params"]["term_lower_bound"]) + " Terms", len(m["M"].iloc[:, 0])
            ),
    ...
    for i in range(2, len(InitialResults) + 2):
        ...
        if norm:
            ax[i - 2, 0].axis(
                [
                    min(res_data["quantileValues"]),
                    max(res_data["quantileValues"]),
                    min(res_data["pdfValues"]),
                    max(res_data["pdfValues"]),
                ]
            )
        ...
        ax[i - 2, 1].plot(
            InitialResults[str(i) + " Terms"]["quantileValues"],
            InitialResults[str(i) + " Terms"]["cumValue"],
            linewidth=2,
        )
        ax[i - 2, 1].axis(
            [
                min(res_data["quantileValues"]),
                max(res_data["quantileValues"]),
                min(res_data["cumValue"]),
                max(res_data["cumValue"]),
            ]
        )
        if i != len(InitialResults) + 1:
...
def update(m, new_data):
    if ((type(new_data) is not list) and (type(new_data) is not np.ndarray)) or (
        not all(isinstance(b, (int, float, np.int_, np.float_)) for b in new_data)
    ):
        raise Exception(
            "Error: 'new_data' must be a numeric vector of type 'list' or 'numpy.ndarray'"
        )

    if m["params"]["save_data"] == False:
        raise Exception(
            "Error: your metalog object does not contain saved data. If you want to update your disritubiton you must set save_data = TRUE in the original distribution creation."
        )
    ...
    v = np.array([])
    for i in range(
        updated_metalog["params"]["term_lower_bound"],
        updated_metalog["params"]["term_limit"] + 1,
    ):
    ...
    b = (
        0.5
        * updated_metalog["params"]["square_residual_error"][
            len(updated_metalog["params"]["square_residual_error"]) - 1
        ]
    )
