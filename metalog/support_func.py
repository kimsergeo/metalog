...
def MLprobs(x, step_len):
    ...
    if len(x) > 100:
        ...
        y = np.append(
            np.append(np.arange(tailstep, np.min(y), tailstep), y),
            np.arange(np.max(y) + tailstep, np.max(y) + 9.001 * tailstep, tailstep),
        )

        if y[0] == 0 or y[0] == 0.0:
            y = np.delete(y, 0)
        elif y[-1] == 1 or y[-1] == 1.0:
            y = np.delete(y, -1)
    ...
    for i in range(len(y)):
        ...
        if term_limit > 4:
            for j in range(5, term_limit + 1):
                ...
                if j % 2 == 0:
                    # iff even
                    diffVector = np.append(
                        diffVector, f ** (e + 1) / d + (e + 1) * f ** e * l
                    )
                    e += 1
...
def newtons_method_metalog(m, q, term):
    ...
    while temp_err > err:
        frist_function = (
            quantileMetalog(
                a, y_now, term, m["params"]["bounds"], m["params"]["boundedness"]
            )
            - q
        )
        derv_function = pdfMetalog(
            a, y_now, term, m["params"]["bounds"], m["params"]["boundedness"]
        )
        ...
        if i > 10000:
            print(
                ...
                q,
                " is too far from distribution median. Try plot() to see distribution.",
            )
...
def a_vector_OLS_and_LP(
    ...
    for i in range(term_lower_bound, term_limit + 1):
        ...
        try:
            temp = np.matmul(
                np.matmul(
                    np.linalg.inv(np.matmul(np.array(Y).T, np.array(Y))), np.array(Y).T
                ),
                np.array(z),
            )
            OLS_error = False
        ...
        if len(z) < 100:
            ...
            y = np.append(
                np.append(
                    np.arange(tailstep, np.min(y) - tailstep + 0.0000000001, tailstep),
                    y,
                ),
                np.arange(np.max(y) + tailstep, np.max(y) + 9.001 * tailstep, tailstep),
            )
        ...
        tempValidation = pd.DataFrame(
            {"term": i, "valid": tempList["valid"], "method": methodFit}, index=[i]
        )
...
def a_vector_LP(
    myList, term_limit, term_lower_bound, diff_error=0.001, diff_step=0.001
):
...
def pdf_quantile_builder(temp, y, term_limit, bounds, boundedness):
    ...
    # Build PDF
    m = np.array(
        [pdfMetalog(temp, y[0], term_limit, bounds=bounds, boundedness=boundedness)]
    )

    for j in range(1, len(y)):
        tempPDF = pdfMetalog(
            temp, y[j], term_limit, bounds=bounds, boundedness=boundedness
        )
    ...
    M = np.array(
        [
            quantileMetalog(
                temp, y[0], term_limit, bounds=bounds, boundedness=boundedness
            )
        ]
    )

    for j in range(1, len(y)):
        tempQuant = quantileMetalog(
            temp, y[j], term_limit, bounds=bounds, boundedness=boundedness
        )
